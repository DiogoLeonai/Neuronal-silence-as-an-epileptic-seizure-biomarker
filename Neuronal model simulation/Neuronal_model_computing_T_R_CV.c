...
int main(void){
  ...
  double *x, tempo, step, *a, *b, *c, *df, *y, *Iext, gg_M, pico[NN + 2], somaISI, somaISI2, tpeak[NN + 2], *Gexc, *Gini, delay, g_exc, g_ini, g;
  double desviopadrao, CV, CV_mean,reobase, iCVsomaISI, iCVsomaISI2, iCVauxISI, iCV, contBurst;
  double CVmedio, contCV, icontUP, icontDOWN, contUP, contUP2, contDOWN, tempoUP, tempoDOWN, ISI, ISI2, contISI;;
  ...
  while(tt<n_n){
    ...
    for (i = 1; i <= NN; i++){
      mean_tempo = mean_tempo + (tempo - tpeak[i]); // <T>
      ...
      if (x[1 + (i - 1) * N] > pico[i]) {
        ...
        if(tempo>transient && k[i]<NMD){ //Saving the spikes and the time they occur for each neuron
        ...
        if (i <= fe * NN)
          for (j = 1; j <= conextotalex[i]; j++)
            Gexc[listaexc[i][j]] = Gexc[listaexc[i][j]] + g_exc;
        else
...
//////////////////// Calculating the Kuramoto order parameter R(t)//////////////////////
  ...
  for(tempo=transient;tempo<=n_n*step;tempo=tempo+2.0*step)
  {
    for(i=1;i<=NN;i=i+1)
      if(kmax[i]>1)
  ...
  for(tempo=transient;tempo<=n_n*step;tempo=tempo+10.0 * step){  
    ISI=0;		  	
    ISI2=0;	
    contISI=0; 	
    ...
    for(i=1;i<=0.1*NN;i=i+1)
      ...
      k[i]=k[i]+1;		         
        ...
        if(contISI>0){	
          ...
          CV = desviopadrao/(ISI/contISI); //CV		
          CVmedio=CVmedio+CV;
          contCV=contCV+1.0;
